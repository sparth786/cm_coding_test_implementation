...
const NewsData: FC<INewsDataType> = (props) => {
    ...
    const [operativeCardData, setOperativeCardData] = useState([]);
    ...
    useEffect(() => {
        newsDataClient.search(searchTerm)
            .then(({hits}) => {
                ...
                setOperativeCardData(getAllHits)
    ...
    const setTopicFiltersHandler = (topic: string) => {
        let localTopicFilters = [...topicFilters]
        if (localTopicFilters.includes(topic)) localTopicFilters = localTopicFilters.filter(val => val !== topic)
        else localTopicFilters = localTopicFilters.concat([topic])

        setTopicFilters(localTopicFilters)

        if (localTopicFilters.length)
            { // @ts-ignore
                setSearchFilteredData(operativeCardData.filter(news => localTopicFilters.some(val => news.topics.map(t => t.title).includes(val))))
            }
        else if (searchTerm) {
            newsDataClient.search(searchTerm)
                .then(({hits}) => {
    ...
    const searchNews = () => {
        ...
        topicFilters.length > 0 && setTopicFilters([]);
    }
                    ...
                    <Grid item xs={3}>
                        <Box>
                            ...
                            <FilterBox
                                ...
                                selectTopicFilters={setTopicFiltersHandler}
                            />
                        </Box>
                    </Grid>
                    <Grid item xs={9}>
